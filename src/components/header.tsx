...
type ExamTab = "written" | "practical";

const WRITTEN_NAV_ITEMS = [
...
const PRACTICAL_NAV_ITEMS = [
  ...
  { path: "/practical/frequency", label: "기출 빈출" },
...
export default function Header() {
  ...
  const activeTab: ExamTab = useMemo(() => {
    if (location.pathname.startsWith("/practical")) return "practical";
    if (location.pathname === "/") return "written";
    return "written";
  }, [location.pathname]);

  const navItems = activeTab === "written" ? WRITTEN_NAV_ITEMS : PRACTICAL_NAV_ITEMS;
        ...
        <div className="flex items-center justify-between h-14">
          <div className="flex items-center gap-3">
            <Link to="/" className="flex items-center gap-2 font-bold text-lg text-gray-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 transition">
              ...
              <span className="hidden sm:inline">정보보안기사</span>
            </Link>
